// src/Android_draw/draw_Gui.cpp —— 最终修复完整版
#include "draw.h"
...
static bool   g_showMenu    = true;    // 修复①：默认弹出菜单
...
static void VideoBallUI() {
    ...
    float bw=96, bh=96; ImTextureID tex=0;
    if (g_videoInited) {
        g_video.Update();
    ...
    if (tex) ImGui::Image(tex, ImVec2(bw,bh));
        ...
        for (int k=0;k<2;k++){ ImVec2 tri[3];
            for (int i=0;i<3;i++){ float a=rot+(k*60+i*120)*3.14159265f/180.0f; tri[i]=ImVec2(c.x+38*cosf(a),c.y+38*sinf(a)); }
    ...
    if (holding&&down){ if(fabsf(io.MousePos.x-downX)+fabsf(io.MousePos.y-downY)>12)dragging=true;
        if(dragging){g_videoPos.x+=io.MouseDelta.x;g_videoPos.y+=io.MouseDelta.y;} }
    if (!down&&prevDown&&holding){ holding=false;
        if(!dragging&&(ImGui::GetTime()-lastToggle)>0.35){g_showMenu=!g_showMenu;lastToggle=ImGui::GetTime();} dragging=false; }
    prevDown = down;
    ImGui::End();
...
static void* HwbpAimThread(void*) {
    while (true) {
        ...
        if (!g_bpSet) {
            ...
            pt.address=(uint64_t)libbase+0x6DFE100; pt.type=PARADISE_HWBP_EXECUTE;
            pt.length=4; pt.scope=PARADISE_HWBP_ALL_THREADS;
...
void DrawPlayerNVG(NVGcontext* vg) {
    ...
    for (int i=0;i<Count;i++){
        ...
        // 修复③：只卡身体中心，留50px余量，去掉r_w条件（近距目标不再被跳过）
...
void DrawCanvas() {
    ...
    DrawPlayerNVG(vg);
    char dbg[96]; snprintf(dbg,sizeof(dbg),"P:%d R:%d B:%d bp:%d tgt:%d set:%d",
        PlayerCount,RealCount,BotCount,(int)g_bpHits,g_Target.valid?1:0,g_bpSet?1:0);
    DrawOutlinedTextNVG(vg,g_font_agency,dbg,{10,120},16.0f,nvgRGBA(0,255,255,255),nvgRGBA(0,0,0,255),false,1.0f);
    ...
    if(g_titleOutline){
        DrawOutlinedTextNVG(vg,g_font_agency,"Asuka追锁 @Asuka1314",{cx,40.0f*UI_SCALE()},40.0f*UI_SCALE(),nvgRGBA(255,255,255,255),nvgRGBA(0,0,0,255),true,2.0f);
